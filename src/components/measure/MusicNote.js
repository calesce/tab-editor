...
export default class MusicNote extends Component {
  ...
  renderWholeNote(x, y, color) {
    return (
      <svg x={x} y={y} style={overflow}>
        <path fill={color} d='M12.768 35.447c-3.296-.174-5.872-3.1-6.906-6.127-.626-1.776-.282-4.44 1.804-4.94 3.023-.513 5.53 2.055 6.937 4.513 1.002 1.822 1.555 4.692-.322 6.137-.445.3-.984.42-1.512.417zm5.252-10.374c-3.714-2.256-8.406-2.432-12.473-1.17C2.97 24.804.024 26.72 0 29.84c-.002 3.055 2.842 4.96 5.358 5.87 3.983 1.302 8.58 1.187 12.298-.89 2.106-1.12 3.972-3.44 3.34-6.01-.35-1.653-1.615-2.89-2.976-3.737z'/>
      </svg>
    );
  }
  ...
  renderThirtySecondNote(x, y, color) {
    return (
      <svg x={x - 8} y={y - 15} style={overflow}>
  ...
  renderDot(x, y, color) {
    return (
      <svg x={x} y={y} width={20} height={20} style={overflow}>
  ...
  renderSharp(x, y, color) {
    return (
      <svg x={x} y={y + 0.5} style={overflow}>
