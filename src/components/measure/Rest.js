...
export default class Rest extends PureComponent {
  ...
  renderQuarterRest(x, y, color) {
    return (
      <svg onClick={this.restClicked} style={style} width='20.53' height='26.7' x={x} y={y}>
        <path
          fill={color}
          ...
          fillRule='evenodd'
        />
      { this.renderDot(14, y - 30, color) }
      </svg>
    );
  }

  renderEighthRest(x, y, color) {
    return (
      <svg onClick={this.restClicked} style={style} width='20.03' height='17.67' x={x} y={y}>
  ...
  renderSixteenthRest(x, y, color) {
    return (
      <svg onClick={this.restClicked} style={style} width='20.66' height='26.65' x={x} y={y}>
        ...
        { this.renderDot(15, y - 30, color) }
      </svg>
    );
  }

  renderHalfRest(x, y, color) {
    return (
      <svg onClick={this.restClicked} style={style} width='30' height='62' x={x - 9} y={y - 5}>
        <g>
        ...
        { this.renderDot(27, y - 25, color) }
      </svg>
    );
  }

  renderWholeRest(x, y, color) {
    return (
      <svg onClick={this.restClicked} style={style} width='30' height='62' x={x - 9} y={y - 12.5}>
        <g transform='matrix(1,0,0,-1,0,42.987635)'>
          <rect fill={color} width='9.2204475' height='5.1031408' x='10.902164' y='18.913252' />
          <rect fill={color} width='15.077462' height='1.1018145' x='7.9446626' y='22.972569' />
        </g>
        { this.renderDot(27, y - 24, color) }
      </svg>
    );
  }
  ...
  render() {
    const { x, y, note, color } = this.props;

    switch(note.duration) {
      case 'q':
        return this.renderQuarterRest(x, y, color);
      case 'e':
        return this.renderEighthRest(x, y, color);
      case 's':
        return this.renderSixteenthRest(x, y, color);
      case 'h':
        return this.renderHalfRest(x, y, color);
      case 'w':
        return this.renderWholeRest(x, y, color);
      default:
        return this.renderQuarterRest(x, y, color);
    }
  }
