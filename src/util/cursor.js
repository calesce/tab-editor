...
export const cursorAfterCutting = (
...
): Cursor =>
  {
    ...
    return { ...oldCursor, noteIndex: noteIndex === 0 ? 0 : noteIndex - 1, measureIndex };
  };
...
export const cursorAfterPasting = (
  ...
  {
    if (clipboard.notes) {
      return { ...oldCursor, noteIndex: oldCursor.noteIndex + clipboard.notes.length };
...
export const getNotesFromSelection = (
...
): Array<Object> | Object =>
  {
    if (selectRange) {
      if (
        Object.keys(selectRange).length === 1 && selectRange[Object.keys(selectRange)[0]] !== 'all'
      ) {
