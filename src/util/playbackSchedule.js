...
export const getRealPlayingIndex = (playingIndex, scheduledSong, currentTrackIndex) => {
  ...
  const newNoteIndex = findIndex(scheduledSong[measureIndex], notes => {
    return notes.filter(
      note => note.trackIndex === currentTrackIndex && note.originalNoteIndex === noteIndex
    ).length;
  });
...
export const getReplaySpeed = (measure, noteIndex, lastNoteIndex) => {
  if (noteIndex === lastNoteIndex) {
    const speeds = measure[noteIndex].map(
      note =>
        getTempoForNote(
  ...
  const speeds = measure[noteIndex].map(
    note => getTempoForNote(getDurationFromPercentage(positionDiff, note.timeSignature), note.tempo)
  );
  return Math.max(...speeds);
};
        ...
        (bucket, note, noteIndex) => {
          ...
          if (bucket.totalDuration + percentage > 1.0) {
            if (bucket.totalDuration < 1.0) {
              ...
              return {
                ...bucket,
                [bucket.totalDuration.toString()]: { ...noteToUse, percentage: newPercentage },
...
export const createCountdownSchedule = (tracks, measureIndex) => {
  return [
    scheduledMeasureArray(trackWithTimeslots([ metronomeTrackForMeasure(tracks, measureIndex) ]))
  ];
};
