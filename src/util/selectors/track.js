...
const tracksWithMeasuresSelector = createSelector(tracksSelector, tracks => {
  return tracks.map(track => ({ ...track, measures: mapMeasureIndices(track.measures) }));
});
...
const findMatchingRepeatBegin = measures => {
  const index = findLastIndex(measures, measure => measure.repeatBegin === true);
...
const expandedTracksFromMeasures = tracks => {
  const repeatEnd = findIndex(tracks[0].measures, measure => measure.repeatEnd === true);
  if (repeatEnd === -1) {
    return tracks;
  }
  const repeatBegin = findMatchingRepeatBegin(tracks[0].measures.slice(0, repeatEnd + 1));
